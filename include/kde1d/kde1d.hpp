...
Kde1d::pdf_continuous(const Eigen::VectorXd& x) const
{
    ...
    if (!std::isnan(xmin_)) {
        fhat =
          (x.array() < xmin_).select(Eigen::VectorXd::Zero(x.size()), fhat);
    }
    if (!std::isnan(xmax_)) {
        fhat =
          (x.array() > xmax_).select(Eigen::VectorXd::Zero(x.size()), fhat);
    }
...
Kde1d::cdf_discrete(const Eigen::VectorXi& x) const
{
    ...
    for (long i = 0; i < x.size(); ++i) {
        out(i) = std::min(1.0, std::max(f_cum(static_cast<size_t>(x(i))), 0.0));
    }
    return out;
}
              ...
              const Eigen::VectorXd& weights)
{
    ...
    res.col(0) =
      res.col(0).array() * (-0.5 * b.array().pow(2) * S.array()).exp();
...
Kde1d::boundary_transform(const Eigen::VectorXd& x, bool inverse)
{
    ...
    if (!inverse) {
        if (!std::isnan(xmin_) & !std::isnan(xmax_)) {
            ...
            x_new =
              (x.array() - xmin_ + 5e-5 * rng) / (xmax_ - xmin_ + 1e-4 * rng);
